/*
C to C-lower compiler for ZaC-3, 32-bit CPU with 32-bit address bus.

C lowerer, outputs "simpler" C for the C->ASM compiler.
This makes the C->ASM compiler much more simple in the asm-gen phase.
E.g.
	a = i++;
	becomes
...
PROBLEMS:
    ...
    - (FIXED) immediate values doesn't work in function (divide(5, 2)), you have to go (divide(a, b)) and make sure a and b have 5 and 2 respectfully.

    - expressions doesn't work on return (return a + b; doesn't work, only return a;).

    - (FIXED) Expressions don't work on compare operators, like "if *(str_a_base + i) == *(str_b_base + i)".
        You have to go 
        char a = *(str_a_base + i);
        char b = *(str_b_base + i);
        if (a == b) { ... }
    
    - (FIXED) you can only write results to a variable from a function if you go "int result = function(...);"
        if you go:
        int result = 0;
        result = function(...);
    ...
    */
    if (current_node->type == AST_MAIN || current_node->type == AST_BODY) {
        ...
        *ast_modifier = current_node->modifier;
        ...
        AST_Node *datatype_node = current_node->getItem(current_node, 0);
        if (symbol_tables[SYMBOL_DATATYPE]->get(symbol_tables[SYMBOL_DATATYPE], datatype_node->ast_string) == INT) {
            sprintf(buffer, "int %s;\n", datatype_node->ast_string);
        } else if (symbol_tables[SYMBOL_DATATYPE]->get(symbol_tables[SYMBOL_DATATYPE], datatype_node->ast_string) == CHAR) {
        ...
        *visibility = symbol_tables[3]->get(symbol_tables[3], current_node->ast_string);
        ...
        char buffer_b[50] = {0};
        extra_stuff->operator_stack_offset = 3;
    ...
    if (current_node->type == AST_MAIN || current_node->type == AST_BODY) {
        ...
        */
        ...
        if (status == ASM_NONE || status == ASM_GET_MEMORY) {
            if (current_node->getItem(current_node, 0)->getItem(current_node->getItem(current_node, 0), 0)->type != AST_VOID) {
                set_stack_param(current_node->getItem(current_node, 0), function_parameters, symbol_tables, pointer_symbol_table, stack, ASM_GET, jmp_label, register_select, pointer_layer_dereference, ast_modifier, visibility, current_function, extra_stuff);
                asm_list->append(asm_list, function_parameters->array);
            }
            sprintf(buffer, "jal _%s\n", current_node->ast_string);
            //extra_stuff->operator_stack_offset = old_operator_stack_offset;
        ...
        if (status == ASM_SET || status == ASM_NONE) {
            //sprintf(buffer+strlen(buffer), "add r%d, r0, r1 ;RETURN_SET\n", extra_stuff->operator_stack_offset);
            sprintf(buffer+strlen(buffer), "movi [sp + %d], r1 ;RETURN_SET_STACK\n", 0);
            if ((char)symbol_tables[9]->get(symbol_tables[9], current_node->ast_string) != 'v') {
                //printf("Non void function detected\n");
                extra_stuff->operator_stack_offset += 1;
            }
