        ...
        if (symbol_tables[6].get(&symbol_tables[6], tail_node->ast_string) > 1) {
            ...
            if (status == ASM_GET) {
                if (symbol_tables[4]->get(symbol_tables[4], tail_node->ast_string) == CHAR) {
                    sprintf(buffer+strlen(buffer), "mvbi r%d, [i0 + 0] ;GET\n", extra_stuff->operator_stack_offset);
                    extra_stuff->operator_stack_offset += 1;
                } else {
                    sprintf(buffer+strlen(buffer), "movi r%d, [i0 + 0] ;GET\n", extra_stuff->operator_stack_offset);
                    extra_stuff->operator_stack_offset += 1;
                }
            } else if (status == ASM_GET_MEMORY) {
                if (symbol_tables[4]->get(symbol_tables[4], tail_node->ast_string) == CHAR) {
                    sprintf(buffer+strlen(buffer), "mvbi r%d, [i0 + 0] ;GET_MEMORY\n", extra_stuff->operator_stack_offset);
                    extra_stuff->operator_stack_offset += 1;
                } else {
                    sprintf(buffer+strlen(buffer), "movi r%d, [i0 + 0] ;GET_MEMORY\n", extra_stuff->operator_stack_offset);
                    extra_stuff->operator_stack_offset += 1;
                }
            } else if (status == ASM_SET) {
                if (symbol_tables[4]->get(symbol_tables[4], tail_node->ast_string) == CHAR) {
                    extra_stuff->operator_stack_offset += -1;
                    sprintf(buffer+strlen(buffer), "mvbi [i0 + 0], r%d ;SET\n", extra_stuff->operator_stack_offset);
                } else {
                    extra_stuff->operator_stack_offset += -1;
                    sprintf(buffer+strlen(buffer), "movi [i0 + 0], r%d ;SET\n", extra_stuff->operator_stack_offset);
                }
            }
        //}
        asm_list->append(asm_list, buffer);
